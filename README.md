...
<p align="center">
  ...
  회사와 집 사이를 이동하는 동안 휴대전화 핫스팟으로 이어가는 macOS 메뉴 막대 앱
</p>
...
1. `/Applications/Runtinue.app`을 실행하고 메뉴 막대의 달리는 사람 아이콘을 엽니다.
2. Wi-Fi를 사용하면 `Wi-Fi 이름 접근 허용…`을 선택합니다.
3. `이동 중 실행 유지…`를 선택합니다.
4. Wi-Fi 핫스팟이나 USB 테더링을 고르고 보호 시간과 연결 대기 시간을 정합니다.
5. Wi-Fi 핫스팟을 처음 사용하면 이름을 확인하고 `내 휴대전화 핫스팟입니다`를 선택합니다.
6. **안전 확인 4개 완료**와 **덮개 닫기 가능**이 함께 나타날 때까지 덮개를 열어 둡니다.
7. 도착하면 `현재 모드 중단`을 누릅니다. 복구 중 안내가 사라지고 `실행 중인 모드 없음`이 표시됐는지 확인합니다.

메뉴의 **언어**에서 시스템 언어, 한국어 또는 영어를 선택할 수 있습니다. 앱 안에서 선택한 언어는 즉시 적용되며 다음 실행에도 유지됩니다. macOS가 띄우는 위치 권한 창은 시스템의 앱 언어 설정을 따릅니다.

시간은 분 단위 정수로 입력합니다. 실행 유지 시간과 연결 대기 한도는 1–1,440분, 활동 종료 후 대기는 1–60분입니다. 문자, 소수와 범위 밖 숫자는 입력 단계에서 거부합니다. 빈칸이나 0을 제출하면 입력 창을 유지한 채 오류를 표시합니다.

<p align="center">
  <img
    src="READMEAssets/trip-start.png"
    width="640"
    alt="핫스팟과 실행 유지 시간, 연결 대기 한도를 설정하는 이동 모드 화면"
  >
</p>
...
| 모드 | 이런 경우에 사용 | 덮개 닫기 |
| --- | --- | --- |
| **이동 모드** (Travel, CLI: `trip`) | 휴대전화 핫스팟이나 USB 테더링으로 전환한 뒤 실행 유지 | 모든 확인이 끝난 뒤 가능 |
| **작업 자동 모드** (Task activity, CLI: `adaptive`) | 연동한 작업 도구의 활동 신호가 있을 때 실행 유지 | 설정과 현재 안전 상태에 따라 결정 |
| **시간 지정 모드** (Timed, CLI: `desk`) | 다운로드나 오래 걸리는 작업 중에 Mac이 잠자기 상태로 전환되지 않도록 함 | 기본은 덮개 열림, 별도 허용 가능 |

**일정 시간 잠자기 방지**의 하위 메뉴에서 **분**(5분, 10분, 15분, 20분, 30분과 45분) 또는 **시간**(1시간, 2시간, 3시간, 4시간, 6시간, 8시간, 10시간, 12시간과 24시간)을 선택하면 별도 입력창 없이 시작합니다. 예를 들어 `일정 시간 잠자기 방지 → 분 → 30분`을 선택하면 30분 동안 Mac이 자동으로 잠자지 않습니다. `현재 모드 중단`을 선택하면 지정한 시간이 지나기 전에도 끝낼 수 있습니다. 프리셋은 덮개를 열고 사용하는 기본값을 따릅니다.

목록에 없는 시간을 사용하거나 덮개 닫기를 허용하려면 같은 하위 메뉴의 **시간과 덮개 설정…**을 선택하십시오. 프리셋은 새 세션을 시작하는 기능이며 실행 중인 세션의 시간을 연장하지 않습니다.

일반 시간 지정 모드는 덮개를 연 상태에서 비활성 상태가 되어도 디스플레이가 꺼지지 않고 Mac이 자동으로 잠자기 상태로 전환되지 않도록 합니다. 분과 시간 프리셋은 물론, **시간과 덮개 설정…**에서 덮개 닫기 옵션을 끈 경우에도 같은 동작을 사용합니다. 지정 시간이 지나거나 사용자가 현재 모드를 중단하거나 안전 조건을 벗어나면 잠자기 방지 해제를 시도합니다. 해제가 확인된 뒤 디스플레이가 다시 자동으로 꺼질 수 있고 Mac도 잠자기 상태로 전환될 수 있습니다.

일반 시간 지정 모드는 화면 유지 요청을 만들 때 macOS에도 유한한 만료 시간을 함께 등록합니다. Supervisor의 관찰이 멈춰도 macOS 전원 관리가 등록된 timeout에 따라 유지 효과를 끌 수 있습니다. Supervisor가 다시 실행되면 기존 종료 기한을 확인하고 토큰 정리를 시도하며 만료된 세션을 다시 켜지 않습니다. 효과의 만료와 토큰 정리는 별개이므로 정리에 실패하면 복구 상태를 유지합니다. OS에 전달하는 남은 시간은 양의 값을 초 단위로 올림하며, 실제 만료 처리에는 OS 스케줄링 지연이 있을 수 있습니다. 만료 순간에 화면을 강제로 끄거나 즉시 잠자기를 요청하는 기능은 아닙니다.

일반 시간 지정 모드는 시작, 주기 관찰과 상태 조회 때마다 디스플레이 유지 요청의 현재 활성 상태를 확인합니다. 확인할 수 없거나 비활성으로 판정되면 지정 시간이 남아 있어도 즉시 해제를 시도합니다. 해제 응답을 기다리는 동안에는 **정상 수면 복구 중**, 해제에 실패하면 **정상 수면 복구 재시도 중**으로 표시하고 새 모드 시작을 막습니다. 나중에 활성 상태를 확인하더라도 중단한 세션을 자동으로 재개하지 않습니다. 해제가 끝난 뒤 사용자가 새 세션을 시작해야 합니다.

시간 지정 모드는 시작 전과 실행 중에 배터리와 macOS 열 압력을 확인합니다. 안전 기준을 벗어나면 지정 시간이 남아 있어도 현재 모드를 중단하고 정상 잠자기 복구를 시도합니다. 이 안전 검사는 덮개 설정과 전원 연결 상태에 맞는 기준을 적용하며 끌 수 없습니다.

이 기능은 macOS의 전원 관리 기능인 [PreventUserIdleDisplaySleep](https://developer.apple.com/documentation/iokit/kiopmassertiontypepreventuseridledisplaysleep)을 사용합니다. 수동 잠자기, 화면 보호기와 보안 정책은 계속 동작합니다. 이미 꺼진 디스플레이를 켜거나 잠금을 해제하는 기능은 없습니다. 별도로 덮개 닫기를 허용한 시간 지정 모드는 기존 전원 처리와 안전 기준을 사용합니다.

이동 중 사용할 때는 **이동 모드**를 선택하십시오. **작업 자동 모드**는 작업 도구에서 `runtinue-hook` 또는 `runtinue-activity`로 활동을 알리는 연동이 필요합니다. 실행 중인 모든 앱을 자동으로 감지하지는 않습니다.

## 화면 상태 읽는 법

이동 모드의 안전 확인 목록은 덮개를 닫기 전에 확인해야 하는 조건을 보여줍니다.
...
일반 시간 지정 세션이 정상 실행 중이면 메뉴 헤더는 **Mac이 잠자지 않도록 하는 중**, 남은 시간, **비활성 상태에서도 디스플레이가 꺼지지 않음**과 **덮개 열기 필요**를 표시합니다. 남은 시간이 1분보다 짧으면 **1분 미만 남음**, 시간값을 확인할 수 없으면 **남은 시간 확인 불가**로 표시합니다. 안전 중단, 복구와 서비스 연결 실패 안내는 축약하지 않으며 추가 상태와 기록 저장 실패 경고도 유지합니다. 배터리와 온도의 자세한 수치는 진단 정보에서 확인할 수 있습니다.

## 기록과 버전 확인

메뉴의 **기록과 진단**에서 최근 기록, 이동 모드 기록 요약, 진단 정보와 새로 고침에 접근할 수 있습니다. 기록 형식과 조회 동작은 그대로이며, 세션을 시작할 수 없는 상태에서도 진단과 새로 고침에 접근할 수 있습니다.

**Runtinue 정보…**는 현재 실행한 메뉴 막대 앱 번들의 버전, 빌드 번호, 소스 커밋과 빌드 당시 커밋되지 않은 변경의 포함 여부를 표시합니다. **버전 정보 복사**는 이 네 항목만 복사하며 SSID, 사용자 경로와 진단 로그를 포함하지 않습니다. 정보창은 인터넷이나 백그라운드 서비스 연결 없이 열 수 있습니다.

메타데이터가 없거나 빌드용 자리표시자라면 정보를 추정하지 않고 확인 불가로 표시합니다. 같은 버전과 빌드 번호라도 소스 커밋이 다를 수 있습니다. 커밋되지 않은 변경을 포함했다면 커밋만으로 해당 빌드를 재현할 수 없으며, 변경이 없다는 표시는 정식 배포나 공증 완료를 뜻하지 않습니다. 이 정보는 메뉴 막대 앱의 식별 정보이지 설치된 모든 서비스의 일치 판정이 아닙니다. Supervisor의 빌드 SHA-256은 기존 진단 정보에서 따로 확인하십시오.

**GitHub 릴리스 보기**는 브라우저에서 릴리스 페이지를 여는 기능입니다. 자동 버전 비교, 최신 버전 판정, 다운로드와 설치를 수행하지 않습니다. 저장소의 `VERSION`이나 최신 커밋을 현재 설치 버전으로 표시하지 않습니다.
...
직접 온도 지원 범위는 `Mac17,8`과 macOS 빌드 `25F84` 조합입니다. 동일 조합의 실기기 1대에서 CPU 전용 부하와 GPU 전용 부하를 따로 주어 CPU 18개와 GPU 7개 센서군의 반응을 확인했습니다. 다른 모델에서는 센서 이름이 비슷해도 추정하지 않고 **이 모델에서 아직 검증되지 않음**으로 표시합니다. 같은 모델이라도 다른 macOS 빌드에서는 **센서 매핑이 검증되지 않음**으로 표시합니다.
...
직접 온도 수집기는 Supervisor의 안전 관측 루프와 독립적으로 실행됩니다. 안전 경로는 마지막으로 완료된 온도 snapshot만 읽으므로 AppleSMC 요청이 지연되더라도 다음 배터리와 macOS 열 압력 관측, lease 갱신과 해제를 기다리게 하지 않습니다.
...
| 조건 | 동작 |
| --- | --- |
| 배터리 사용, 외부 화면 없음, 덮개 닫힘, 배터리 30% 미만 | 현재 모드 중단 |
| AC 연결 상태이며 외부 화면이 없고 덮개가 닫힌 상태에서, 충전하지 않거나 충전 표시 중 관찰된 첫 배터리 감소가 다음 표본에서도 회복되지 않음 | 30% 기준 적용 |
| 충전 중 배터리 10% 미만 | 비상 기준으로 현재 모드 중단 |
| 배터리 정보 확인 불가 | 보호 제한 |
| 외부 화면 없음, 덮개 닫힘, macOS 열 압력 `fair` 이상 | 전원 연결과 관계없이 현재 모드 중단 |
...
덮개를 열어 두고 `기록과 진단 → 새로 고침`을 실행한 뒤 `기록과 진단 → 진단 정보…`를 확인하십시오. Wi-Fi 이름을 읽지 못하면 `Wi-Fi 감지 권한 허용`을 선택하거나 시스템 설정에서 Runtinue의 위치 권한을 허용하십시오.
...
메뉴 막대 앱을 종료해도 Runtinue 백그라운드 서비스가 관리하는 활성 모드는 계속 실행될 수 있습니다. 먼저 `현재 모드 중단`이나 `runtinue stop`을 사용하십시오.

<details>
<summary><strong>이전 SafeClam 설치에서 전환하기</strong></summary>

SafeClam으로 설치한 버전이 남아 있으면 Runtinue 설치가 중단됩니다. 모든 보호 모드를 중단하고 SafeClam 메뉴 막대 앱을 종료한 뒤 기존 제거 도구를 실행하십시오.
...
메뉴 막대 앱과 CLI는 같은 Runtinue 백그라운드 서비스 상태를 사용합니다.
...
일반 시간 지정 경로의 `UserPowerAssertionBackend.acquire(reason:deadline:)`에는 같은 부팅 범위의 연속 시계로 계산한 종료 기한을 전달합니다. 구현은 그 기한까지 남은 시간과 `kIOPMAssertionTimeoutActionTurnOff`를 `IOPMAssertionCreateWithProperties`의 한 요청에 포함합니다. 커스텀 backend도 관찰 호출 없이 효과가 만료되고 이후 소유한 토큰을 해제할 수 있어야 합니다. 이미 지났거나 최대 24시간을 넘는 기한은 전원 요청 전에 거부합니다.
...
프로세스 간 통신은 프로토콜 6을 사용합니다. 앱, CLI, Supervisor, Helper를 같은 패키지로 교체해야 합니다. XPC 호출자는 macOS의 연결 단위 코드 서명 검증을 사용합니다. Runtinue는 성공 응답을 받더라도 lease ID, 사용자 UID, 실제 시스템 수면 설정과 유효 기한이 모두 일치할 때만 유효한 결과로 인정합니다.
