...
일반 시간 지정 모드는 덮개를 연 상태에서 비활성 상태가 되어도 디스플레이가 꺼지지 않고 Mac이 자동으로 잠자기 상태로 전환되지 않도록 합니다. 분과 시간 프리셋은 물론, **시간과 덮개 설정…**에서 덮개 닫기 옵션을 끈 경우에도 같은 동작을 사용합니다. 지정 시간이 지나거나 사용자가 현재 모드를 중단하거나 안전 조건을 벗어나면 디스플레이가 다시 자동으로 꺼질 수 있고 Mac도 잠자기 상태로 전환될 수 있습니다.
