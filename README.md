...
사용자가 휴대전화와 macOS에서 연결을 바꾸면 앱이 결과를 확인합니다. 앱은 Wi-Fi에 직접 접속하지 않습니다. 입력한 핫스팟 이름은 다음 시작 폼의 기본값으로 먼저 저장되므로 첫 시도가 실패해도 남을 수 있습니다. 실제 보호까지 성공한 대상만 별도의 네트워크 지문으로 검증하며, 실패한 입력은 기존 검증 대상을 덮어쓰지 않습니다. 핫스팟 전환 후 네트워크가 잠시 끊겨도 활성 lease를 그 이유만으로 종료하지 않고 기기 안전 조건과 시간 제한을 계속 적용합니다.
...
| AC 연결 중 충전하지 않거나 충전 표시 중 배터리가 두 차례 연속 감소, 외부 화면 없음, 덮개 닫힘 | 30% 기준 적용 |
...
저장소 작업 원칙, 검증 절차와 공개 파일 경계는 [AGENTS.md](AGENTS.md)를 따릅니다. 새 공개 경로가 필요하면 허용 목록과 회귀 테스트를 같은 변경에서 갱신하십시오.
