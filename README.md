...
<p align="center">
  <img
    ...
    width="640"
    alt="네트워크, 인터넷, 기기 상태와 수면 보호 확인을 마치고 덮개 닫기 가능 상태를 표시한 Runtinue 메뉴"
  >
</p>
...
> [!IMPORTANT]
> **현재 버전 0.2.2는 실험 단계입니다.** Apple Silicon MacBook과 macOS 13 이상을 지원합니다. 공개 GitHub Release나 서명과 공증을 마친 설치 패키지는 아직 없습니다. 자동화 테스트를 구성했습니다. 모든 MacBook과 실제 통근 환경을 대상으로 한 공개 검증은 아직 완료하지 않았습니다.
...
3. **기기**: 배터리, 전원, 열 상태와 덮개 상태를 확인합니다.
...
6. 보호 진행선(Flowline)의 네 단계가 끝나고 **덮개 닫기 가능**이 나타날 때까지 덮개를 열어 둡니다.
...
Trip 상태의 보호 진행선(Flowline)은 `네트워크 → 인터넷 → 기기 → 보호` 순서를 보여줍니다. 중단과 복구 중에는 `수면 보호 → 정상 수면` 순서로 바뀝니다.
...
- 열 상태가 기준 이상으로 높아진 경우
- 배터리, 열 상태와 덮개 상태를 확인할 수 없는 경우
...
<details>
<summary><strong>배터리와 열 상태 기준 보기</strong></summary>
...
| AC 연결 상태이며 외부 화면이 없고 덮개가 닫힌 상태에서, 충전하지 않거나 충전 표시 중 배터리가 두 차례 연속 감소 | 30% 기준 적용 |
...
- 앱은 macOS의 열 상태를 사용하며 부품과 외장 표면 온도를 직접 측정하지 않습니다. 센서 지연, 운영체제 오류, 프로세스 장애로 보호 로직이 실패할 수 있습니다. 장시간 고부하 연산, 대규모 빌드, 로컬 모델 추론을 피하십시오.
