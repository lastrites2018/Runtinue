...
7. 도착하면 `실행 유지 중단`을 누릅니다. 복구 중 안내가 사라지고 `실행 유지 꺼짐`이 표시됐는지 확인합니다.
...
| **시간 지정 모드** (Timed, CLI: `desk`) | 다운로드나 긴 작업을 정해진 시간 동안 실행 | 기본은 덮개 열림, 별도 허용 가능 |

**시간을 정해 유지**의 하위 메뉴에서 **분**(5분, 10분, 15분, 20분, 30분과 45분) 또는 **시간**(1시간, 2시간, 3시간, 4시간, 6시간과 8시간)을 선택하면 별도 입력창 없이 시작합니다. 예를 들어 `시간을 정해 유지 → 분 → 30분`으로 30분 유지를 시작하고, 기존 `실행 유지 중단`으로 끝낼 수 있습니다. 프리셋은 덮개를 열고 사용하는 기본값을 따릅니다.

목록에 없는 시간이나 기존 옵션을 지정하려면 같은 하위 메뉴의 **직접 입력…**을 선택하십시오. 기존 시간 지정 입력창과 검증을 그대로 사용합니다. 프리셋은 새 세션을 시작하는 기능이며 실행 중인 세션의 시간을 연장하지 않습니다.

일반 시간 지정 모드는 덮개를 연 상태에서 **화면 자동 꺼짐과 시스템의 유휴 잠자기를 함께 방지**합니다. 분과 시간 프리셋 및 덮개 닫힘을 허용하지 않은 직접 입력이 같은 동작을 사용합니다. 지정 시간이 끝나거나 사용자가 중단하거나 안전 조건을 벗어나면 기존 해제 경로로 화면 유지 요청을 해제합니다. 이 동작은 화면이 꺼진 상태로 작업만 유지하던 기존 일반 시간 지정 모드와 다릅니다.

화면 유지는 [macOS의 디스플레이 유휴 수면 방지 assertion](https://developer.apple.com/documentation/iokit/kiopmassertiontypepreventuseridledisplaysleep)을 사용합니다. 수동 잠금, 화면 보호기와 보안 정책을 우회하지 않으며 이미 꺼진 화면을 켜거나 잠금을 해제하지 않습니다. 이동 모드와 별도로 허용한 덮개 닫힘 보호는 기존 전원 처리와 안전 기준을 유지합니다.
...
일반 시간 지정 세션이 정상 실행 중이면 메뉴 헤더는 **실행 유지 중**, 남은 시간과 **덮개를 열고 사용**을 표시합니다. 남은 시간이 1분보다 짧으면 **1분 미만 남음**, 시간값을 확인할 수 없으면 **남은 시간 확인 불가**로 표시합니다. 안전 중단, 복구와 서비스 연결 실패 안내는 축약하지 않으며 추가 상태와 기록 저장 실패 경고도 유지합니다. 배터리와 온도의 자세한 수치는 진단 정보에서 확인할 수 있습니다.

## 기록과 버전 확인

메뉴의 **기록과 진단**에서 최근 기록, 이동 모드 기록 요약, 진단 정보와 새로 고침에 접근할 수 있습니다. 기록 형식과 조회 동작은 그대로이며, 세션을 시작할 수 없는 상태에서도 진단과 새로 고침에 접근할 수 있습니다.

**Runtinue 정보…**는 현재 실행한 메뉴바 앱 번들의 버전, 빌드 번호, 소스 커밋과 빌드 당시 커밋되지 않은 변경의 포함 여부를 표시합니다. **버전 정보 복사**는 이 네 항목만 복사하며 SSID, 사용자 경로와 진단 로그를 포함하지 않습니다. 정보창은 인터넷이나 백그라운드 서비스 연결 없이 열 수 있습니다.

메타데이터가 없거나 빌드용 자리표시자라면 정보를 추정하지 않고 확인 불가로 표시합니다. 같은 버전과 빌드 번호라도 소스 커밋이 다를 수 있습니다. 커밋되지 않은 변경을 포함했다면 커밋만으로 해당 빌드를 재현할 수 없으며, 변경이 없다는 표시는 정식 배포나 공증 완료를 뜻하지 않습니다. 이 정보는 메뉴바 앱의 식별 정보이지 설치된 모든 서비스의 일치 판정이 아닙니다. Supervisor의 빌드 SHA-256은 기존 진단 정보에서 따로 확인하십시오.
