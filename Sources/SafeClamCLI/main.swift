...
struct SafeClamCLI {
  ...
  private static func helperServiceIsLoaded() -> Bool {
    ...
    process.arguments = ["print", "system/\(SafeClamIPCContract.helperMachServiceName)"]
    process.environment = [
      "LANG": "C",
      "LC_ALL": "C",
  ...
  private static func probePrivilegedHelperDirectly() async -> HelperBoundaryProbeOutcome {
    await withCheckedContinuation { continuation in
      let connection = NSXPCConnection(
        machServiceName: SafeClamIPCContract.helperMachServiceName,
        options: .privileged
      )
  ...
  private static func printUsage() {
    print(
      """
      사용법:
        safeclam trip start --for <시간> (--hotspot <SSID> | --usb-tether) [--handoff-timeout <시간>]
        safeclam start --for <시간> (--hotspot <SSID> | --usb-tether) [--handoff-timeout <시간>]
        safeclam status [--json]
        safeclam stop
        safeclam history [--limit <개수>]
        safeclam events
        safeclam diagnose
        safeclam verify-helper-boundary
        safeclam adaptive enable [--idle-grace <시간>] --max <시간>
        safeclam adaptive disable
        safeclam desk enable --max <시간> [--closed-lid]
        safeclam desk disable
        safeclam inspect
        safeclam watch-handoff --hotspot <SSID> [--timeout-seconds <초>]
