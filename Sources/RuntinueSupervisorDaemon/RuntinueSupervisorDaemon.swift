...
@main
struct RuntinueSupervisorDaemon {
  static func main() async {
    guard geteuid() != 0 else {
      FileHandle.standardError.write(
        Data("runtinue-supervisor must run in a user session\n".utf8)
      )
      Foundation.exit(77)
    }
    ...
    let controlListener = NSXPCListener(
      machServiceName: RuntinueIPCContract.supervisorMachServiceName
    )
    ...
    let activityListener = NSXPCListener(
      machServiceName: RuntinueIPCContract.supervisorActivityMachServiceName
    )
