...
runtime=NO
rollback=NO
install_root=${SAFECLAM_INSTALL_ROOT:-/}

usage() {
  print -u2 "사용법: verify-installation.sh <manifest.json> [--pkg <SafeClam.pkg>] [--runtime] [--rollback]"
  exit 64
}
...
[[ "${identifier}" == "com.example.safeclam.pkg" ]] || fail "manifest package identifier 불일치" 65
...
if [[ "${install_root}" != /* ]]; then
  fail "SAFECLAM_INSTALL_ROOT는 절대 경로여야 함" 64
fi
if [[ "${install_root}" != "/" && "${SAFECLAM_INSTALL_FIXTURE:-NO}" != YES ]]; then
  fail "실제 설치가 아닌 root는 SAFECLAM_INSTALL_FIXTURE=YES에서만 허용" 64
fi
...
verify_file artifacts.safeclam "/usr/local/bin/safeclam"
verify_file artifacts.safeclamHook "/usr/local/bin/safeclam-hook"
verify_file artifacts.safeclamActivity "/usr/local/bin/safeclam-activity"
verify_file artifacts.safeclamHelper \
  "/Library/Application Support/com.example.safeclam/bin/safeclam-helper"
verify_file artifacts.safeclamSupervisor \
  "/Library/Application Support/com.example.safeclam/bin/safeclam-supervisor"
verify_file artifacts.safeclamMenubar \
  "/Applications/SafeClam.app/Contents/MacOS/safeclam-menubar"
verify_file artifacts.safeclamAppInfo \
  "/Applications/SafeClam.app/Contents/Info.plist"
verify_file artifacts.safeclamAppCodeResources \
...
supervisor_hash=$(manifest_value artifacts.safeclamSupervisor.sha256) || fail "Supervisor hash 누락" 65
...
if [[ "${install_root}" == "/" ]]; then
  receipt_info=$(/usr/sbin/pkgutil --pkg-info com.example.safeclam.pkg 2>/dev/null) || \
...
if [[ "${runtime}" == YES ]]; then
  ...
  /bin/launchctl print system/com.example.safeclam.helper >/dev/null 2>&1 || \
    fail "Helper launchd 상태 검증 실패" 67
  # root 설치 검사와 사용자 XPC의 인증 주체를 섞지 않는다.
  console_cli() {
    if [[ "${EUID}" -eq 0 ]]; then
      /bin/launchctl asuser "${console_uid}" \
        /usr/bin/sudo -n -u "#${console_uid}" /usr/local/bin/safeclam "$@"
